<div class="row padding-1 p-1">
    <div class="col-md-12">
        ...
        <div class="form-group mb-2 mb20">
            ...
            {!! $errors->first('fecha_finalizacion', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
        ...
        <div class="form-group mb-2 mb20">
            ...
            {!! $errors->first('id_etapa', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>

        <div class="form-group mb-2 mb20">
            <label for="id_usuario" class="form-label">{{ __('Historial Clínico Asociado') }}</label>
            <select name="id_usuario" id="id_usuario"
                ...
                @foreach ($usuarios as $usuario)
                    <option value="{{ $usuario->id }}"
                        {{ old('id_usuario', $progresoVoluntario->id_usuario ?? '') == $usuario->id ? 'selected' : '' }}>
                        {{ $usuario->id }} - {{ $usuario->email ?? 'Sin email' }}
                    </option>
                @endforeach
            </select>
